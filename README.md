...
- 顶部“AI 设置”入口用于配置模型服务商、模型与 API Key；未配置 AI 时，本地 Task 草稿解析仍可完整使用。
...
- 第四个业务页“随手记”提供标题、富文本编辑区和按最后更新时间倒序排列的笔记列表，支持标题/正文搜索、修改标题、星标收藏、收藏夹筛选、删除双重确认和未保存修改提醒。
- 编辑器支持加粗、斜体、12/14/16/18/22 五档字号、SharePoint/HTTP/HTTPS 链接，以及类似 Excel 的 20 色字色和 20 色底色预设盘；不引入任意取色器、图片或附件。
- 可直接粘贴 Excel 表格并保留合并单元格；选中矩形区域或通过表格侧面的浮动按钮选择整表后，复制回 Excel 仍保持表格结构和合并关系。
- 表格工具采用“新建表格 / 编辑表格”二级菜单和具体操作三级菜单；支持尺寸面板新建表格、在当前格下方插行、右侧插列、删除当前行列、删除整个表格和合并矩形选区。横向、纵向及矩形选择都有明确高亮，不会误标相邻格文字。
- 通过表格侧面按钮选中整表后，Windows 下 `Delete` 清空单元格内容、`Backspace` 删除整表；macOS 下普通 `Delete` 键清空内容、`Fn+Delete` 删除整表。清空和删除都可撤销。
- 新建、粘贴、插入、删除、合并及删除最后一行/列均进入浏览器原生编辑历史，可使用 `Command/Ctrl + Z` 撤销并重做。AI 改写会用占位保护表格，改写后恢复原表格而不会将其删除。

![随手记表格与收藏夹](readme配图/笔记表格与收藏.png)

![随手记 AI 改写前后对照](readme配图/笔记编辑.png)

- 图中展示已启用的“AI 转换”、AI 改写结果和可恢复的“改写前原文”；AI 结果始终需要用户检查并保存。
