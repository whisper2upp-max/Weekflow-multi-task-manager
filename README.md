...
- 编辑器支持加粗、斜体、SharePoint/HTTP/HTTPS 链接，以及类似 Excel 的 20 色字色和 20 色底色预设盘；不引入任意取色器、图片、附件或表格。
...
- “转换为 Task 草稿”完全在本地以确定性中英文规则识别一个或多个潜在 Task，不调用 AI 或网络。按独立行书写的 `1 / 2 / 3`、`1. / 2. / 3.` 或中文序号默认拆成不同 Task 候选；明确标签和完整日期可可靠预填。
...
v2.7 为避免迁移或清空既有用户数据，继续沿用 v2.4 数据命名空间，并把内部结构升级到 v4；这只是兼容性存储键，不代表当前程序版本仍是 v2.4。
...
- 最新版本（v2.7）更新时间：2026年8月15日

## 程序文件

- 本文件夹是 Weekflow v2.7 空白正式发布版。
- 程序需要保留整个文件夹结构，不能只移动 `Weekflow.html`；页面依赖 `css、js、templates、vendor` 子目录。
- 如果收到 `Weekflow v2.7.zip`，请完整解压后再使用。
