...
从 `/Users/cici/Desktop/AI/project/task manager kimi`（Electron 版 Weekflow 3.0）移植而来：目标是把安装/分发体积从 277MB 级降到 10–20MB 级，**功能与 UI 完全不变**。Electron 版仅作只读参考，**不要修改它任何文件**。
...
- 持久化：Rust 侧 JSON 文件（数据目录 `~/Library/Application Support/weekflow-tauri/weekflow-data.json`）+ **自动轮换备份**（`backups/`，保留最近 30 份）
...
src/
  ...
  renderer/      与 Electron 版逐字相同，**唯一差异**：
...
src-tauri/       Rust 后端
  ...
  tauri.conf.json  productName Weekflow、identifier com.weekflow.app、
                   窗口 1440x900（min 960x640）、CSP
...
- `npm run build`：tauri build（产出 `.app` 与 `.dmg`，在 `src-tauri/target/release/bundle/`）
- 手动冒烟：`WEEKFLOW_USER_DATA_DIR=$(mktemp -d) "src-tauri/target/release/bundle/macos/Weekflow.app/Contents/MacOS/Weekflow"`，15 秒不退出、数据文件被创建即 OK。
...
渲染进程只通过 `window.weekflow` 访问原生能力（接口定义 `src/shared/ipc.ts` 的 `WeekflowApi`，Tauri 版由 `src/renderer/lib/tauri-bridge.ts` 实现）：数据读写、文件保存/打开对话框（导入导出 xlsx/json，二进制跨桥走 base64）、`openExternal`（打开资料链接，桥接层校验仅 http/https，底层用 `@tauri-apps/plugin-opener` 的 `openUrl`）。
...
13. 使用说明/更新日志弹窗内容照搬原 HTML（含"开发团队：Wesley Yan"、v1.0–v2.5 日志）。
