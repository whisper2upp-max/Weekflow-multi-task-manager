...
(function (root, factory) {
  ...
  function translateAttribute(name, value) {
    ...
    var attributes = {
      ...
      ,"Weekflow v3.0 中英双语本地 Task 管理工具，支持周/日双时间轴、随手记、多条进度历史、可选 AI 改写与 Task 草稿解析、资料库、看板、备份和 Excel 批量导入导出。": "Weekflow v3.0 bilingual local Task manager with weekly/daily timelines, Quick Notes, multi-entry progress history, optional AI rewriting and Task-draft parsing, a Document Library, dashboards, backup, and Excel import/export."
  ...
  function englishGuideHtml() {
    return [
      ...
      '<section><h3>Development Team</h3><p>Developer: Wesley Yan</p><p>First release (v1.0): July 30, 2026</p><p>Latest release (v3.0): August 16, 2026</p></section>'
    ].join("");
  }

  function englishChangelogHtml() {
    var entries = [
      ["v3.0 Quick Notes, Progress History, and AI Assistance", "2026-08-16", "Added Quick Notes, reusable multi-entry Task progress history, deterministic local Task-draft conversion, and optional multi-provider AI assistance.", ["Added titled rich-text-only Notes with links, search, latest-updated sorting, unsaved-change protection, and double-confirm deletion.", "Added 12/14/16/18/22 font-size presets and two Excel-like 20-color text/highlight palettes to Notes and progress history while preserving forward and reverse selections.", "Added optional multi-provider AI Settings, current-Note rewriting, and AI-first Task-draft parsing; deterministic bilingual local rules remain available without an API Key or network connection.", "Automatically falls back to local parsing after AI failures or timeouts; every result stays in the existing sequential review form, while cross-Group Flows and impossible dates are rejected.", "Prevents late AI rewrite results from overwriting another Note or newer edits, keeps provider-specific request fields isolated, and stores the API Key outside business-data JSON backups.", "Added one-time Note-to-progress conversion and sequential multi-candidate Task-draft review with source split view, skip, save-and-continue, and manual Add Task Draft.", "Used bilingual deterministic rules to split numbered and ordinary non-empty lines while keeping labeled detail lines attached; precise date prefixes and recurring phrases prefill safe fields for user confirmation.", "Upgraded JSON to v4 with Notes and progressEntries while migrating legacy progressNote content into one history entry.", "Added one-row-per-entry Progress History to re-importable data, dashboard reports, and person reports; kept main-cell newline aggregation and Excel's 32,767-character limit.", "Stabilized all five English navigation tabs and retained unfrozen Windows-safe three-sheet OOXML plus all timeline/document scroll-position safeguards."]],
